    ...
    for(;;)
    {
        ...
        if (0u != USBUART_GetConfiguration()){
            ...
            if (0u != USBUART_DataIsReady()){
                ...
                if(bufferSwitch == 0){
                    //count += USBUART_GetAll(RxBuffer0 + count);
                    count += USBUART_ReadOutEP(3, &RxBuffer0[count], 64);
                    while(USBUART_OUT_BUFFER_FULL == USBUART_GetEPState(3));
                    USBUART_EnableOutEP(3);
                    LCD_Position(0, 14);
                    LCD_PrintNumber(USBUART_GetEPCount(3));
                    //if (0u != count){
                        ...
                        */
                        if ((USBUART_BUFFER_SIZE == count)){
                            ...
                            while (0u == USBUART_CDCIsReady()){}
                            USBUART_LoadInEP(2, RxBuffer0, 64);
                            
                            /* Wait until component is ready to send data to PC. */
                            while (0u == USBUART_CDCIsReady()){}
                            
                            /* Send zero-length packet to PC. */
                            USBUART_LoadInEP(2, NULL, 0u); 
                            /* Reset Count and Switch RX Buffer */
                            count = 0;
                            bufferSwitch = 1;
                        }
                    //}
                }else{
                    count += USBUART_ReadOutEP(3, &RxBuffer1[count], 64);
                    while(USBUART_OUT_BUFFER_FULL == USBUART_GetEPState(3));
                    USBUART_EnableOutEP(3);
                    //if (0u != count){
                        /* Wait until component is ready to send data to host. */
                        ...
                        */
                        if((USBUART_BUFFER_SIZE == count)){
                            /* Send data back to host. */
                            while (0u == USBUART_CDCIsReady()){}
                            USBUART_LoadInEP(2, RxBuffer1, 64);
